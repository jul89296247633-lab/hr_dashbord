...
- **4 KPI-карточки** `grid grid-cols-4 gap-4` (иконки: `Phone` Звонки, `Users` Собеседования, `UserCheck` Выведено, `GraduationCap` На стажировке):
  - Каждая: иконка + название + «факт / план» + `Progress` shadcn
  - Карточка «На стажировке»: счётчик без плана, только факт (стажировка — промежуточный этап)
